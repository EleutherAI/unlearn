...
def eval_lab_bench(model, tokenizer):
    ...
    for i in range(len(litqa2['train'])):
        q, a = format_question(litqa2['train']['question'][i], litqa2['train']['ideal'][i], litqa2['train']['distractors'][i], litqa2['train']['key-passage'][i], 'ABCD'[i%4])
        if q is not None:
    ...
    for i in range(len(protocolqa['train'])):
        q, a = format_question(protocolqa['train']['question'][i], protocolqa['train']['ideal'][i], protocolqa['train']['distractors'][i], protocolqa['train']['protocol'][i], 'ABCD'[i%4])
        if q is not None:
    ...
    with torch.no_grad():
        for i in range(len(all_questions)):
            tokenized_question = tokenizer(all_questions[i], return_tensors='pt', padding='do_not_pad').to(model.device)
...
if __name__ == '__main__':
    ...
    for eval in args.evals:
        if eval == 'lab_bench':
            ...
            performance = lm_eval_model(model, task=eval, tokenizer=tokenizer, revision=args.revision, system_instruction=args.system_instruction)
