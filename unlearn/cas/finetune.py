...
class WMDPEvalCallback(TrainerCallback):
    ...
    def on_step_begin(self, args, state, control, **kwargs):
        if state.global_step % self.eval_every == 0:
            wmdp_acc = lm_eval_model(self.model, task='wmdp_bio_aisi', revision=self.run_args.revision, tokenizer=self.tokenizer)
            cloze = lm_eval_model(self.model, task='wmdp_bio_aisi_cloze_verified', revision=self.run_args.revision, tokenizer=self.tokenizer)
            wmdp_acc['wmdp_bio_aisi_cloze_verified'] = cloze['wmdp_bio_aisi_cloze_verified']
...
if __name__ == '__main__':
    ...
    if args.include_chat_retain:
        ...
        retain_chat_dataset = retain_chat_dataset.shuffle(seed=42).select(range(args.num_train_examples))
    ...
    if args.include_text_retain:
        ...
        retain_text_dataset = retain_text_dataset.shuffle(seed=42).select(range(args.num_train_examples))
    ...
    if args.include_refusal_retain:
        ...
        retain_refusal_compliance_dataset = retain_refusal_compliance_dataset.shuffle(seed=42).select(range(args.num_train_examples))
    ...
    if args.include_incompetent_compliance_retain:
        ...
        retain_incompetent_compliance_dataset = retain_incompetent_compliance_dataset.shuffle(seed=42).select(range(args.num_train_examples))
    ...
    if args.include_bio_retain:
        ...
        bio_retain_dataset = bio_retain_dataset['train'].shuffle(seed=42).select(range(args.num_train_examples))
    ...
    if args.include_bio_remove:
        ...
        bio_remove_dataset = bio_remove_dataset['train'].shuffle(seed=42).select(range(args.num_train_examples))
    ...
    if args.include_corrupt_rewritten:
        ...
        corrupt_dataset = corrupt_dataset['train'].shuffle(seed=42).select(range(args.num_train_examples))
