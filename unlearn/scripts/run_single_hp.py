...
def append_to_results(results_file: str, orth_coef: float, remove_coef: float,
                      wmdp_acc: float, mmlu_acc: float, notes: str = ""):
...
def main():
    ...
    if train_result.returncode != 0:
        ...
        append_to_results(str(results_file), args.orth_coef, args.remove_coef, -1.0, -1.0, "Training failed")
        sys.exit(1)
    ...
    wmdp_cmd = [
        "python", "-m", "unlearn.evaluation.eval_wmdp_robust",
    ...
    wmdp_result = subprocess.run(wmdp_cmd, cwd=str(PROJECT_ROOT), capture_output=True, text=True)
    ...
    mmlu_result = subprocess.run(mmlu_cmd, cwd=str(PROJECT_ROOT), capture_output=True, text=True)
    ...
    append_to_results(str(results_file), args.orth_coef, args.remove_coef, wmdp_acc, mmlu_acc)

    print(f"\n[{datetime.now()}] Done!")
    print(f"Results: orth_coef={args.orth_coef}, remove_coef={args.remove_coef}, WMDP={wmdp_acc:.4f}, MMLU={mmlu_acc:.4f}")
