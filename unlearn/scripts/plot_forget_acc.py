...
def main():
    ...
    parser.add_argument("--retain_coef", type=float, default=5, help="Retain coefficient used")
    parser.add_argument("--remove_coef", type=float, default=8, help="Remove coefficient used")
    ...
    if window > 1:
        ...
        plt.plot(smoothed_steps, smoothed, 'r-', linewidth=2, label=f'Smoothed ({window}-epoch window)')

    plt.axhline(y=0.85, color='g', linestyle='--', label='Target: 85%')
    plt.axhline(y=forget_acc[-1], color='orange', linestyle=':', label=f'Final: {forget_acc[-1]*100:.1f}%')

    plt.xlabel('Training Steps', fontsize=12)
    plt.ylabel('Forget Accuracy', fontsize=12)
    plt.title(f'Checkpoint Transfer with KL Retain Loss\nretain_coef={args.retain_coef}, remove_coef={args.remove_coef}, {args.epochs} epochs ({total_steps} steps)', fontsize=14)
    ...
    if args.output:
        ...
        output_path = Path(args.log_file).parent.parent / 'experiment_logs' / 'forget_acc_over_training.png'
